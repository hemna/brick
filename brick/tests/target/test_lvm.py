...
from brick.target.local_dev import lvm as brick
...
class BrickLvmTestCase(test.TestCase):
    ...
    def setUp(self):
        self.configuration = mock.MagicMock(name='configuration')
        ...
        mock_path = "brick.openstack.common.processutils.execute"
        with mock.patch(mock_path, side_effect=self.fake_execute):
    ...
    def test_create_lv_snapshot(self):
        mock_path = "brick.openstack.common.processutils.execute"
        with mock.patch(mock_path, side_effect=self.fake_execute):
            self.assertEqual(self.vg.create_lv_snapshot('snapshot-1',
                                                        'fake-1'),
                             None)

            get_volume_mock = mock.Mock()
            get_volume_mock.return_value = None
            self.vg.get_volume = get_volume_mock
            try:
    ...
    def test_thin_support(self):
        ...
        with mock.patch(mock_path, side_effect=self.fake_pretend_lvm_version):
            self.assertTrue(self.vg.supports_thin_provisioning('sudo'))

        with mock.patch(mock_path, side_effect=self.fake_old_lvm_version):
    ...
    def test_thin_pool_creation(self):
        ...
        with mock.patch(mock_path, side_effect=self.fake_execute):
            ...
            # Passing a size parameter should result in a thin pool
            # of that exact size.
            for size in ("1g", "1.2g", "1.75g"):
                self.assertEqual(size,
                                 self.vg.create_thin_pool(size_str=size))
